...
from visionllm.vargpt.var_model.models import VAR, VQVAE, build_vae_vargpt, build_vae_vargpt_v1
from visionllm.vargpt.var_model.models.helpers import sample_with_top_k_top_p_

from visionllm.vargpt_qwen_v1_1.var_model.tools.run_infinity import *
from visionllm.vargpt_qwen_v1_1.var_model.infinity.models.bitwise_self_correction import BitwiseSelfCorrection
from visionllm.vargpt_qwen_v1_1.var_model.infinity.models.infinity import sample_with_top_k_top_p_also_inplace_modifying_logits_
...
class Qwen2VLCausalLMOutputWithPast(ModelOutput):
    ...
    image_gen_labels: torch.Tensor = None
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        # import ipdb; ipdb.set_trace()
        # TODO: add support for attention_mask on 
        # if gen_image_config is not None:
        #     image_gen_length = gen_image_config['image_gen_length']
        #     gen_attn_mask = gen_image_config['gen_attn_mask']
        #     # 2. 根据 start_idx 替换 causal_mask 中的对应部分
        #     start_idx = gen_image_config['start_idx']
        #     gen_idx = 0  # gen_attn_mask 的索引
        #     for i in range(len(start_idx)):
        #         if start_idx[i] != -1:  # 如果当前 batch 存在有效的 start_idx
        #             start = start_idx[i]
        #             for j in range(len(start)):
        #                 causal_mask[i, :, start[j]+1:start[j]+1 + image_gen_length, start[j]+1:start[j]+1+ image_gen_length] = gen_attn_mask
        #                 gen_idx += 1  # 更新 gen_attn_mask 的索引
        # elif inference_image_gen:
        #     causal_mask = None
...
class VARGPTQwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        training_gen_stage = 2
        if training_gen_stage == 1:
            self.training_pn = '0.06M'
        elif training_gen_stage == 2:
            self.training_pn = '0.25M'
        elif training_gen_stage == 3:
            self.training_pn = '1M'
            
        else:
            assert False, f'training_gen_stage={training_gen_stage}'
    ...
    def process_image_gen_tokens(
        ...
        other_labels[image_gen_mask] = -100  # 忽略 image_gen_token 的位置
    ...
    def forward(
        ...
        rope_deltas: Optional[torch.LongTensor] = None,
        inference_image_gen: Optional[bool] = False,
        dpo_training: Optional[bool] = False,
        ...
        if pixel_gen_values is not None and self.model.training and self.version == 0.1:
            ...
            logits = other_logits
            gen_logits = image_gen_logits
        elif pixel_gen_values is not None and (self.model.training or dpo_training) and self.version == 1.1:
        ...
        return Qwen2VLCausalLMOutputWithPast(
            ...
            image_gen_labels = image_gen_labels,
