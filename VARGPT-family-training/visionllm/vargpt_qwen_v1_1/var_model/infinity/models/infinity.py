...
import visionllm.vargpt_qwen_v1_1.var_model.infinity.utils.dist as dist
from visionllm.vargpt_qwen_v1_1.var_model.infinity.utils.dist import for_visualize
from visionllm.vargpt_qwen_v1_1.var_model.infinity.models.basic import flash_fused_op_installed, AdaLNBeforeHead, CrossAttnBlock, SelfAttnBlock, CrossAttention, FastRMSNorm, precompute_rope2d_freqs_grid
from visionllm.vargpt_qwen_v1_1.var_model.infinity.utils import misc
from visionllm.vargpt_qwen_v1_1.var_model.infinity.models.flex_attn import FlexAttn
from visionllm.vargpt_qwen_v1_1.var_model.infinity.utils.dynamic_resolution import dynamic_resolution_h_w, h_div_w_templates

try:
    from visionllm.vargpt_qwen_v1_1.var_model.infinity.models.fused_op import fused_ada_layer_norm, fused_ada_rms_norm
except:
...
class Infinity(nn.Module):
    def __init__(
        ...
        self.customized_flash_attn = customized_flash_attn and False
    ...
    def get_logits(self, h: torch.Tensor, cond_BD: Optional[torch.Tensor]):
        ...
        return self.head(self.head_nm(h.to(dtype=cond_BD.dtype), cond_BD))
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        # print('x_BLC_wo_prefix.dtype:', x_BLC_wo_prefix.dtype)
        if cfg_infer:
        ...
        sos = cond_BD = self.text_proj_for_sos((kv_compact, cu_seqlens_k, max_seqlen_k)).contiguous()    # cond_BD should be float32 torch.Size([2, 2048])
        kv_compact = self.text_proj_for_ca(kv_compact).contiguous()
        kv_compact[0, 0] += must_on_graph
        ca_kv = kv_compact, cu_seqlens_k, max_seqlen_k # torch.Size([144, 2048]), tensor([  0, 128, 144], device='cuda:0', dtype=torch.int32), 128
        ...
        # print('sos.dtype:', sos.dtype, 'cond_BD_or_gss.dtype:', cond_BD_or_gss.dtype, "x_BLC_wo_prefix.dtype:", x_BLC_wo_prefix.dtype)
        ...
        return  x_BLC, ca_kv, cond_BD_or_gss, cond_BD,  attn_bias_or_two_vector, attn_fn, scale_schedule, need_to_pad, l_end
    
    def forward_decoder(self, x_BLC, ca_kv, cond_BD_or_gss, cond_BD,  attn_bias_or_two_vector, attn_fn, scale_schedule, need_to_pad, l_end):
        # [2. block loop]
        SelfAttnBlock.forward, CrossAttnBlock.forward
        # print("x_BLC.dtype", x_BLC.dtype, "cond_BD_or_gss.dtype", cond_BD_or_gss.dtype, "cond_BD.dtype", cond_BD.dtype, "attn_bias_or_two_vector.dtype", attn_bias_or_two_vector.dtype, "ca_kv[0]", ca_kv[0].dtype) #"
        # print("self.rope2d_freqs_grid.dtype",{v.dtype for k, v in self.rope2d_freqs_grid.items()})
        self.rope2d_freqs_grid = {k: v.to(x_BLC.dtype) for k, v in self.rope2d_freqs_grid.items()}
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                ...
                if not self.add_lvl_embeding_only_first_block:
                    x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
                # print("x_BLC.dtype", x_BLC.dtype, "cond_BD_or_gss.dtype", cond_BD_or_gss.dtype, "attn_fn", attn_fn.dtype, "cond_BD.dtype", cond_BD.dtype, "attn_bias_or_two_vector.dtype", attn_bias_or_two_vector.dtype, "ca_kv[0]", ca_kv[0].dtype) #"
                # print("self.rope2d_freqs_grid.dtype",{v.dtype for k, v in self.rope2d_freqs_grid.items()})
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            must_on_graph = self.cfg_uncond[0, 0] * 0
            kv_compact = self.text_norm(kv_compact).contiguous()
            sos = cond_BD = self.text_proj_for_sos((kv_compact, cu_seqlens_k, max_seqlen_k)).float().contiguous()    # cond_BD should be float32 torch.Size([2, 2048])
            kv_compact = self.text_proj_for_ca(kv_compact).contiguous()
            kv_compact[0, 0] += must_on_graph
            ca_kv = kv_compact, cu_seqlens_k, max_seqlen_k # torch.Size([144, 2048]), tensor([  0, 128, 144], device='cuda:0', dtype=torch.int32), 128
