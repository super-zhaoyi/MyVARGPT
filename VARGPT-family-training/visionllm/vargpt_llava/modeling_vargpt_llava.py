...
from visionllm.vargpt_llava.var_model.models import VAR, VQVAE, build_vae_vargpt, build_vae_vargpt_v1
from visionllm.vargpt_llava.var_model.models.helpers import sample_with_top_k_top_p_
import os
...
_GOLBAL_FILE_INDEX_ = int(os.environ.get('_GLOBAL_FILE_INDEX_', '0'))
_GOLBAL_IMAGE_INDEX_ = 0
...
class VARGPTLlavaForConditionalGeneration(LlavaPreTrainedModel, GenerationMixin):
    def __init__(self, config: VARGPTLlavaConfig):
    ...
    def forward(
        ...
        pixel_gen_values: Optional[torch.FloatTensor] = None,
        ...
        if pixel_gen_values is not None and not any(x is None for x in pixel_gen_values):
            gt_BL, x_BLCv_wo_first_l = self.get_vae_gt_xin(pixel_gen_values) 
            ...
            x_BLC, attn_bias, cond_BD_or_gss, cond_BD = self.vargpt_gen(label_image_gen_start, x_BLCv_wo_first_l) 
        ...
        if legacy_processing:
            assert False, "Legacy processing is deprecated in image generation mode." 
        ...
        global _GOLBAL_FILE_INDEX_, _GOLBAL_IMAGE_INDEX_
        if inference_image_gen:
            ...
            for si, pn in enumerate(self.vargpt_gen.patch_nums):   # si: i-th segment
                ...
                # cfg
                gaussian_noise = torch.randn(B, hidden_states.shape[1], hidden_states.shape[2], device=hidden_states.device)  # 保持设备一致
                ...
                if not more_smooth: # this is the default case
                    ...
                    h_BChw = gumbel_softmax_with_rng(logits_BlV.mul(1 + ratio), tau=gum_t, hard=False, dim=-1, rng=rng) @ self.vargpt_gen.vae_quant_proxy[0].embedding.weight.unsqueeze(0)
            ...
            chw.save(f'./image_output/{_GOLBAL_FILE_INDEX_}/{_GOLBAL_IMAGE_INDEX_}_output.png')
        else:    
            outputs = self.language_model(
