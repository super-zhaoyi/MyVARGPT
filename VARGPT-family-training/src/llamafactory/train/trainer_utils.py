...
def get_batch_image_logps(
    logits: "torch.Tensor", 
    labels: "torch.Tensor",
    bitloss_type: str = 'sum'
) -> Tuple["torch.Tensor", "torch.Tensor"]:
    """
    Computes the log probabilities for image generation.
    
    Args:
        logits: Shape (batch_size, seq_len, channel) where channel = bits*2
        labels: Shape (batch_size, seq_len, bits)
        bitloss_type: Type of bit loss aggregation ('mean' or 'sum')
    
    Returns:
        log_probs: A tensor of shape (batch_size,) containing the log probabilities
        valid_length: A tensor of shape (batch_size,) containing sequence lengths
    """
    batch_size, seq_len, channel = logits.shape
    bits = labels.shape[-1]
    
    # 重塑logits为(batch_size, seq_len, bits, 2)并调整维度顺序为(batch_size, 2, seq_len, bits)
    reshaped_logits = logits.reshape(batch_size, seq_len, bits, 2).permute(0, 3, 1, 2).float()
    
    # 计算log softmax
    log_probs = F.log_softmax(reshaped_logits, dim=1)
    
    # 获取对应标签位置的log概率
    # labels: [batch_size, seq_len, bits] -> [batch_size, 1, seq_len, bits]
    labels = labels.unsqueeze(1)
    log_probs = torch.gather(log_probs, 1, labels).squeeze(1)  # [batch_size, seq_len, bits]
    
    # 聚合bits维度的概率
    if bitloss_type == 'mean':
        log_probs = log_probs.mean(dim=-1)  # [batch_size, seq_len]
    elif bitloss_type == 'sum':
        log_probs = log_probs.sum(dim=-1)   # [batch_size, seq_len]
    else:
        raise ValueError(f"Unsupported bitloss_type: {bitloss_type}")
    
    # 计算每个序列的总对数概率和有效长度
    valid_length = torch.tensor([seq_len] * batch_size, device=logits.device)
    batch_log_probs = log_probs.sum(dim=-1)  # [batch_size]
    
    return batch_log_probs, valid_length