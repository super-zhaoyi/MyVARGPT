...
def run_sft(
    ...
    print("compute_dtype",  model_args.compute_dtype)
    ...
    def debug_collator(features):
        batch = data_collator(features) #Batch keys: dict_keys(['input_ids', 'attention_mask', 'labels', 'pixel_values', 'image_grid_thw'])
        ...
        for i in range(min(1, len(batch['input_ids']))):
            print(f"\nSample {i}:")
    ...
    if finetuning_args.include_effective_tokens_per_second:
        for data in dataset_module["train_dataset"]: #Dataset({ features: ['input_ids', 'attention_mask', 'labels', 'images', 'videos'], num_rows: 5 })
