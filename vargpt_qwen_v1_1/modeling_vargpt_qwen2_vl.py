...
from vargpt_qwen_v1_1.var_model.tools.run_infinity import *
from vargpt_qwen_v1_1.var_model.infinity.models.bitwise_self_correction import BitwiseSelfCorrection
from vargpt_qwen_v1_1.var_model.infinity.models.infinity import sample_with_top_k_top_p_also_inplace_modifying_logits_
...
class VARGPTQwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self._IMAGE_GEN_PATH = "output.png"
    ...
    def autoregressive_infer_cfg(
        ...
        if self.model.dtype == torch.bfloat16:
            cond_BD_or_gss = self.vargpt_gen.shared_ada_lin(cond_BD).contiguous()
        else:
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if vae_type != 0:
                ...
                if si < gt_leak:
                    idx_Bl = gt_ls_Bl[si]
                if self.model.dtype == torch.bfloat16:
                    h_BChw = self.vargpt_gen.quant_only_used_in_inference[0].embedding(idx_Bl)   # BlC
            ...
            last_stage = last_stage.to(self.model.dtype)
            if si != num_stages_minus_1:
        ...
        summed_codes = summed_codes.to(self.model.dtype)
        if vae_type != 0:
    ...
    def forward(
        ...
        if inference_image_gen:
            ...
            cv2.imwrite(f'{self._IMAGE_GEN_PATH}', generated_image.cpu().numpy())
